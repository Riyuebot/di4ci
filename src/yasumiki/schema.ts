...
const 当前路线值 = ['未定', '黄泉线', '机知线', '暗黑线', '神明线'] as const;
const 当前阵营值 = ['未知', '人类', '狼'] as const;
const 当前胜负状态值 = ['未分出', '人类胜利', '狼胜利'] as const;
const 宴会当前阶段值 = ['未开始', '讨论中', '票决中', '已散场'] as const;
const 角色生存状态值 = ['存活', '已失踪', '被隔离', '被关押', '已死亡'] as const;
...
export const 去重字符串列表 = () =>
  z
    .array(z.any())
    .transform(values => _.uniq(_.flattenDeep(values).map(value => 归一化文本(value)).filter(Boolean)))
    .prefault([]);
...
export const 创建角色功能身份 = (初始值 = '村民') => 创建字符串标准化器(初始值, 功能身份别名映射);
...
export const 宴会当前阶段 = 创建枚举标准化器(宴会当前阶段值, '未开始', {
  未开宴: '未开始',
  未开启: '未开始',
  进行中: '讨论中',
  正式讨论: '讨论中',
...
export const 当前胜负状态 = 创建枚举标准化器(当前胜负状态值, '未分出', {
  平民胜利: '人类胜利',
  村民胜利: '人类胜利',
  人类阵营胜利: '人类胜利',
  狼人胜利: '狼胜利',
...
export const 角色生存状态 = 创建枚举标准化器(角色生存状态值, '存活', {
  正常: '存活',
  存活中: '存活',
  失踪: '已失踪',
  失联: '已失踪',
...
export const SchemaObject = z.object({
  ...
  生存: z
    .object({
      角色存活状态: z
        .object({
          芹泽千枝实: 角色生存状态.prefault('存活'),
          回末李花子: 角色生存状态.prefault('存活'),
          马宫久子: 角色生存状态.prefault('存活'),
          织部香织: 角色生存状态.prefault('存活'),
          卷岛春: 角色生存状态.prefault('存活'),
          咩子: 角色生存状态.prefault('存活'),
          山胁多惠: 角色生存状态.prefault('存活'),
          美佐峰美辻: 角色生存状态.prefault('存活'),
          织部泰长: 角色生存状态.prefault('存活'),
          织部义次: 角色生存状态.prefault('存活'),
          酿田近望: 角色生存状态.prefault('存活'),
          室匠: 角色生存状态.prefault('存活'),
          能里清之介: 角色生存状态.prefault('存活'),
          卷岛宽造: 角色生存状态.prefault('存活'),
          狼爷爷: 角色生存状态.prefault('存活'),
          桥本雄大: 角色生存状态.prefault('存活'),
        })
        .prefault({}),
    })
    .prefault({}),
  ...
  永久记忆: 永久记忆结构,
  永久Key: 旧永久Key兼容结构.optional().prefault({}),
